...
import { Settings as SettingsIcon, User, Store, Printer, Bell, Key, X, Upload, Image as ImageIcon, Database, Download, UploadCloud, Trash2, Info, Phone, Code, Cloud, RefreshCw, Check, AlertCircle, Plus, Edit2, UserX, UserCheck, Shield, ShoppingBag, Package, MessageCircle, FileText, Clock, Filter, Volume2, VolumeX, Music, Play } from 'lucide-react'
...
// Chat Notification Settings Component
function ChatNotificationSettings() {
  const { chatNotification, updateChatNotification } = useSettingsStore()
  const [settings, setSettings] = useState(chatNotification || {
    enabled: true,
    soundEnabled: true,
  ...
  const [customSoundUrlInput, setCustomSoundUrlInput] = useState(settings.customSoundUrl || '')
  ...
  const handleSoundFileChange = async (e) => {
    const file = e.target.files[0]
    if (!file) return
    
    try {
      const soundUrl = await setCustomSound(file)
      setCustomSoundFile(file.name)
      setSettings(prev => ({ ...prev, soundId: 'custom', customSoundUrl: soundUrl }))
    } catch (error) {
      alert(error.message)
    }
  }
  ...
  const handleCustomUrlSave = () => {
    if (customSoundUrlInput) {
      setCustomSoundUrl(customSoundUrlInput)
      setSettings(prev => ({ ...prev, soundId: 'custom', customSoundUrl: customSoundUrlInput }))
      alert('URL sound berhasil disimpan!')
    }
  }
  
  const handleSelectSound = (soundId) => {
    selectDefaultSound(soundId)
    setSettings(prev => ({ ...prev, soundId, customSoundUrl: null }))
  ...
  const handleRequestPermission = async () => {
    ...
    if (permission === 'granted') {
      ...
      alert('Notifikasi browser ditolak. Silakan aktifkan manual di pengaturan browser.')
    }
  }
  
  const handleSave = () => {
    updateChatNotification(settings)
    alert('Pengaturan notifikasi chat berhasil disimpan!')
  }
  ...
  return (
    <div className="card">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-lg font-bold">Notifikasi Chat Marketplace</h3>
      </div>
      ...
      <div className="space-y-4">
        ...
        <label className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Bunyi saat ada chat baru</p>
          </div>
        </label>
        ...
        {settings.soundEnabled && (
          <div className="space-y-3 pl-8 border-l-2 border-gray-200">
            <p className="text-sm font-medium text-gray-700">Pilih Suara:</p>
            ...
            {/* Custom Sound Upload */}
            <div className="mt-3">
              <p className="text-sm font-medium text-gray-700 mb-2">Atau Upload Sound Custom:</p>
              <div className="flex items-center gap-2">
                ...
                {customSoundFile && (
                  <span className="text-sm text-green-600">✓ {customSoundFile}</span>
                )}
              </div>
              <p className="text-xs text-gray-400 mt-1">Format: MP3, WAV, OGG (Max 500KB)</p>
            </div>
            
            {/* Custom Sound URL */}
            <div className="mt-3">
              <p className="text-sm font-medium text-gray-700 mb-2">Atau Masukkan URL Sound:</p>
              <div className="flex items-center gap-2">
                <input
                  type="url"
                  value={customSoundUrlInput}
                  onChange={(e) => setCustomSoundUrlInput(e.target.value)}
        ...
        <div className="border-t pt-4 mt-4">
          <label className="flex items-center gap-3">
            <input 
              ...
              onChange={(e) => setSettings(prev => ({ ...prev, browserNotification: e.target.checked }))}
              className="w-5 h-5" 
            />
            <div>
              <span className="font-medium">Notifikasi Browser</span>
              <p className="text-xs text-gray-500">Tampilkan popup notifikasi di browser</p>
            </div>
          </label>
          
          {settings.browserNotification && notifPermission !== 'granted' && (
            <div className="mt-2 pl-8">
              ...
              {notifPermission === 'denied' && (
                <p className="text-xs text-red-500 mt-1">
                  Notifikasi diblokir. Silakan aktifkan manual di pengaturan browser.
                </p>
              )}
            </div>
          )}
          {notifPermission === 'granted' && (
            <p className="text-xs text-green-600 pl-8 mt-1">✓ Notifikasi browser aktif</p>
          )}
        </div>
        ...
        <div className="border-t pt-4 mt-4">
          <label className="block">
            <span className="text-sm font-medium text-gray-700">Interval Auto-Refresh Chat</span>
            <select
