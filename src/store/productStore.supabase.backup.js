...
export const useProductStore = create(
  persist(
    (set, get) => ({
      ...
      // initRealtime is kept for backward compatibility but delegates to central sync
      initRealtime: async () => {
        if (!isSupabaseConfigured()) {
          console.log('Supabase not configured, using local storage only')
          return
        }
        // Realtime is now handled by useRealtimeSync.js
        // Just mark as online if Supabase is configured
        set({ isOnline: true })
        console.log('📦 ProductStore: Using central realtime sync from useRealtimeSync.js')
      },
      ...
      addProduct: async (product, userId = null, userName = null) => {
        ...
        if (isSupabaseConfigured()) {
          const { error } = await supabase.from('products').insert({
            ...
            cost: newProduct.cost || newProduct.costPrice || 0,
      ...
      updateProduct: async (id, updatedProduct, userId = null, userName = null) => {
        ...
        if (isSupabaseConfigured()) {
          const { error } = await supabase.from('products').update({
            ...
            cost: updatedProduct.cost || updatedProduct.costPrice,
...
function transformProduct(row) {
  return {
    ...
    unit: row.unit || row.unit_id,
    price: parseFloat(row.price || row.selling_price) || 0,
    cost: parseFloat(row.cost || row.cost_price) || 0,
    costPrice: parseFloat(row.cost || row.cost_price) || 0,
    stock: row.stock || 0,
    minStock: row.min_stock || 0,
    maxStock: row.max_stock || 0,
    image: row.image_url,
    parentId: row.parent_id || row.parent_product_id,
